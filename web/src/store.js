...
const store = new Vuex.Store({
  state: {
    ...
    currentFileId: null, // 当前编辑的文件ID（服务端文件）
    aiConfig: {
      // 大模型服务配置（新）
      provider: 'custom',  // 服务商类型：openai, azure, volcengine, zhipu, qianfan, ollama, xinference, custom
      baseUrl: 'https://ark.cn-beijing.volces.com/api/v3',
      apiKey: '',
      model: '',
      enableThinking: false,  // 是否开启思考模式
  ...
  mutations: {
    ...
    setLocalConfig(state, data) {
      ...
      // 新字段 → 旧字段（保存时保持兼容）
      if (data.baseUrl && !data.api) {
        data.api = data.baseUrl
      }
